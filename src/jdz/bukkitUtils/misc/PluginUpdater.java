...
public class PluginUpdater {
	 ...
	 * @param announce
	 *            True if the program should announce the progress of new updates in
	 *            console.
	 */
	 ...
	 *            console.
	 */
	public PluginUpdater(Plugin plugin, int id, File file, UpdateType type, UpdateCallback callback, boolean announce) {
		this.plugin = plugin;
		this.type = type;
	 ...
	 * necessary to wait for the thread to finish before allowing anyone to check
	 * the result.
	 */
	...
	/**
	 * Check if the name of a jar is one of the plugins currently installed, used
	 * for extracting the correct files out of a zip.
	 *
	 * @param name
	...
	/**
	 * Check to see if the program should continue by evaluating whether the plugin
	 * is already updated, or shouldn't be updated.
	 *
	 * @return true if the version was located and is not the same as the remote's
	 *         newest.
	 */
	 ...
	 * available on BukkitDev which is not this version is indeed an "update". If a
	 * version is present on BukkitDev that is not the version that is currently
	 * running, Updater will assume that it is a newer version. This is because
	 * there is no standard versioning scheme, and creating a calculation that can
	 * determine whether a new update is actually an update is sometimes extremely
	 * complicated.
	 * </p>
	 * <p>
	 * Updater will call this method from {@link #versionCheck()} before deciding
	 * whether the remote version is actually an update. If you have a specific
	 * versioning scheme with which a mathematical determination can be reliably
	 * made to decide whether one version is higher than another, you may revise
	 * this method, using the local and remote version parameters, to execute the
	 * appropriate check.
	 * </p>
	 * <p>
	 * Returning a value of <b>false</b> will tell the update process that this is
	 * NOT a new version. Without revision, this method will always consider a
	 ...
	 * @param remoteVersion
	 *            the remote version
	 * @return true if Updater should consider the remote version an update, false
	 *         if not.
	 */
	...
	/**
	 * Evaluate whether the version number is marked showing that it should not be
	...
	/**
	 * Called on main thread when the Updater has finished working, regardless of
	 * result.
	 */
